...
exports.navigate = exports.useClientRouter = void 0;
const utils_1 = require("../../shared/utils");
const navigationState_1 = require("../navigationState");
const throttle_1 = require("../../shared/utils/throttle");
...
const urlFullOriginal = (0, utils_1.getUrlFull)();
function useClientRouter({ render, onTransitionStart, onTransitionEnd }) {
    (0, utils_1.assertUsage)(isAlreadyCalled === false, '`useClientRouter` can be called only once.');
    ...
    async function fetchAndRender(scrollTarget, url = (0, utils_1.getUrlFull)()) {
        ...
        const pageContext = await (0, getPageContext_1.getPageContext)(url, navigationState_1.navigationState.noNavigationChangeYet);
        ...
        changeUrl(url);
        navigationState_1.navigationState.markNavigationChange();
        (0, utils_1.assert)(renderPromise === undefined);
        renderPromise = (async () => {
            (0, utils_1.objectAssign)(pageContext, { isHydration: isFirstPageRender && url === urlFullOriginal });
...
exports.useClientRouter = useClientRouter;
...
async function navigate(url, { keepScrollPosition = false } = {}) {
    ...
    (0, utils_1.assertUsage)(navigateFunction, '[navigate()] You need to call `useClientRouter()` before being able to use `navigate()`.');
...
let urlFullWithoutHash__previous = (0, utils_1.getUrlFullWithoutHash)();
function onBrowserHistoryNavigation(callback) {
    window.addEventListener('popstate', (ev) => {
        // Skip hash changes
        const urlFullWithoutHash__current = (0, utils_1.getUrlFullWithoutHash)();
...
function changeUrl(url) {
    if ((0, utils_1.getUrlFull)() === url)
        return;
    browserNativeScrollRestoration_disable();
    window.history.pushState(undefined, '', url);
    urlFullWithoutHash__previous = (0, utils_1.getUrlFullWithoutHash)();
}
...
function setScrollPosition(scrollTarget) {
    ...
    if (scrollTarget === 'scroll-to-top-or-hash') {
        ...
        (0, utils_1.assert)('x' in scrollTarget && 'y' in scrollTarget);
...
function getScrollPositionFromHistory(historyState = window.history.state) {
    return (0, utils_1.hasProp)(historyState, 'scrollPosition') ? historyState.scrollPosition : null;
}
function autoSaveScrollPosition() {
    ...
    window.addEventListener('scroll', (0, throttle_1.throttle)(saveScrollPosition, 100), { passive: true });
...
function getUrlHash() {
    ...
    if (hash === '')
        return null;
    (0, utils_1.assert)(hash.startsWith('#'));
    hash = hash.slice(1);
    return hash;
}
