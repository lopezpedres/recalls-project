 ...
 */
async function prerender({ partial = false, noExtraDir = false, root = process.cwd(), clientRouter = false, parallel = (0, os_1.cpus)().length, base } = {}) {
    assertArguments(partial, noExtraDir, clientRouter, base, root, parallel);
    console.log(`${(0, kolorist_1.cyan)(`vite-plugin-ssr ${utils_1.projectInfo.version}`)} ${(0, kolorist_1.green)('pre-rendering HTML...')}`);
...
async function routeAndPrerender(globalContext, htmlFiles, prerenderPageIds, concurrencyLimit, noExtraDir) {
    ...
    await Promise.all(globalContext.prerenderPageContexts.map((pageContext) => concurrencyLimit(async () => {
        ...
        const pageContextRouteAddendum = await (0, route_1.route)(pageContext);
        (0, utils_1.assert)((0, utils_1.hasProp)(pageContextRouteAddendum, '_pageId', 'null') || (0, utils_1.hasProp)(pageContextRouteAddendum, '_pageId', 'string'));
        ...
        (0, utils_1.assert)(pageContextRouteAddendum._pageId);
...
async function writeHtmlFile({ url, htmlString, pageContextSerialized, doNotCreateExtraDirectory, pageId }, root, doNotPrerenderList, concurrencyLimit) {
    (0, utils_1.assert)(url.startsWith('/'));
    ...
    if (pageContextSerialized !== null) {
        writeJobs.push(write(url, '.pageContext.json', pageContextSerialized, root, doNotCreateExtraDirectory, concurrencyLimit));
    }
    await Promise.all(writeJobs);
}
function write(url, fileExtension, fileContent, root, doNotCreateExtraDirectory, concurrencyLimit) {
    return concurrencyLimit(async () => {
        ...
        const filePath = (0, path_1.join)(root, 'dist', 'client', filePathRelative);
        await mkdirp((0, path_1.dirname)(filePath));
        await writeFile(filePath, fileContent);
        console.log(`${(0, kolorist_1.gray)((0, path_1.join)('dist', 'client') + path_1.sep)}${(0, kolorist_1.blue)(filePathRelative)}`);
    });
}
...
function getPluginManifest(root) {
    ...
    (0, utils_1.assertUsage)((0, moduleExists_1.moduleExists)(pluginManifestPath), "You are trying to run `$ vite-plugin-ssr prerender` but you didn't build your app yet: make sure to run `$ vite build && vite build --ssr` before running the pre-rendering. (Following build manifest is missing: `" +
        pluginManifestPath +
        '`.)');
    let manifestContent = require(pluginManifestPath);
    (0, utils_1.assert)((0, utils_1.hasProp)(manifestContent, 'version'));
    (0, utils_1.assert)((0, utils_1.hasProp)(manifestContent, 'base'));
    (0, utils_1.assert)((0, utils_1.hasProp)(manifestContent, 'usesClientRouter'));
    const { base, usesClientRouter } = manifestContent;
    (0, utils_1.assert)(typeof usesClientRouter === 'boolean');
    (0, utils_1.assert)(typeof base === 'string');
    const pluginManifest = { version: utils_1.projectInfo.version, base, usesClientRouter };
    (0, utils_1.assertUsage)(pluginManifest.version === utils_1.projectInfo.version, `Remove \`dist/\` and re-build your app \`$ vite build && vite build --ssr && vite-plugin-ssr prerender\`. (You are using \`vite-plugin-ssr@${utils_1.projectInfo.version}\` but your build has been generated with a different version \`vite-plugin-ssr@${pluginManifest.version}\`.)`);
    return pluginManifest;
}
function assertArguments(partial, noExtraDir, clientRouter, base, root, parallel) {
