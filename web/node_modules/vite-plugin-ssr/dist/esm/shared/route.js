...
import { assert, assertUsage, higherFirst, slice, hasProp, isPlainObject, isPromise, objectAssign, isObjectWithKeys, assertExports } from './utils';
...
async function resolveRouteFunction(pageRouteFileExports, urlPathname, pageContext, pageRouteFilePath) {
    ...
    assertUsage(isPlainObject(result), `The Route Function ${pageRouteFilePath} should return a boolean or a plain JavaScript object, instead it returns \`${result && result.constructor}\`.`);
    if (!hasProp(result, 'match')) {
        result['match'] = true;
    }
    assert(hasProp(result, 'match'));
    assertUsage(typeof result.match === 'boolean' || typeof result.match === 'number', `The \`match\` value returned by the Route Function ${pageRouteFilePath} should be a boolean or a number.`);
    ...
    Object.keys(result).forEach((key) => {
        assertUsage(key === 'match' || key === 'routeParams', `The Route Function ${pageRouteFilePath} returned an object with an unknown key \`{ ${key} }\`. Allowed keys: ['match', 'routeParams'].`);
    });
    assert(isPlainObject(routeParams));
    return {
