...
function getPageContextProxy(pageContext) {
    return new Proxy(pageContext, { get });
    function get(_, prop) {
        if (typeof prop === 'symbol')
            return pageContext[prop]; // Vue tries to access some symbols
        if (typeof prop !== 'string')
            return pageContext[prop];
        if (prop.startsWith('__v_'))
            return pageContext[prop]; // Vue internals upon `reactive(pageContext)`
        if (prop in pageContext || SKIP.includes(prop))
            return pageContext[prop];
        assertUsage(false, [
...
//# sourceMappingURL=getPageContextProxy.js.map