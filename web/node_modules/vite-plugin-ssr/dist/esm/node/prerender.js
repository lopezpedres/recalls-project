...
import { getGlobalContext, loadOnBeforePrerenderHook, loadPageFiles, prerenderPage, renderStatic404Page } from './renderPage';
 ...
 */
async function prerender({ partial = false, noExtraDir = false, root = process.cwd(), clientRouter = false, parallel = cpus().length, base } = {}) {
...
async function routeAndPrerender(globalContext, htmlFiles, prerenderPageIds, concurrencyLimit, noExtraDir) {
    ...
    await Promise.all(globalContext.prerenderPageContexts.map((pageContext) => concurrencyLimit(async () => {
        ...
        assert(hasProp(pageContextRouteAddendum, '_pageId', 'null') || hasProp(pageContextRouteAddendum, '_pageId', 'string'));
...
function getPluginManifest(root) {
    ...
    assertUsage(pluginManifest.version === projectInfo.version, `Remove \`dist/\` and re-build your app \`$ vite build && vite build --ssr && vite-plugin-ssr prerender\`. (You are using \`vite-plugin-ssr@${projectInfo.version}\` but your build has been generated with a different version \`vite-plugin-ssr@${pluginManifest.version}\`.)`);
