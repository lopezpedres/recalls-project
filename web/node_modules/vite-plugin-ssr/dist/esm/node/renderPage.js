import { getErrorPageId, getAllPageIds, route, isErrorPage, loadPageRoutes } from '../shared/route';
import { isEscapeInject, renderEscapeInject, getHtmlString } from './html/escapeInject';
import { getAllPageFiles_serverSide, findPageFile, findDefaultFiles } from '../shared/getPageFiles';
import { getSsrEnv } from './ssrEnv';
import { posix as pathPosix } from 'path';
import { stringify } from '@brillout/json-s';
import { assert, assertUsage, lowerFirst, isCallable, cast, assertWarning, hasProp, handlePageContextRequestSuffix, getUrlPathname, isPlainObject, isObject, getUrlParsed, objectAssign, compareString, assertExports, stringifyStringArray, handleUrlOrigin } from '../shared/utils';
...
import { getNodeStream, getWebStream, pipeToStreamWritableWeb, pipeToStreamWritableNode } from './html/stream';
export { renderPage };
...
async function renderPage(pageContext) {
    ...
    */
    ...
    objectAssign(globalContext, { _isPreRendering: false });
...
function createHttpResponseObject(escapeResult, { statusCode, renderFilePath }) {
    ...
    return {
        ...
        get bodyNodeStream() {
            ...
            assertUsage(nodeStream !== null, '`pageContext.httpResponse.bodyNodeStream` is not available: make sure your `render()` hook provides a Node.js Stream, see https://vite-plugin-ssr.com/html-streaming');
            return nodeStream;
        },
        get bodyWebStream() {
            assert(escapeResult !== null);
            const webStream = getWebStream(escapeResult);
            assertUsage(webStream !== null, '`pageContext.httpResponse.bodyWebStream` is not available: make sure your `render()` hook provides a Web Stream, see https://vite-plugin-ssr.com/html-streaming');
            return webStream;
        },
        bodyPipeToWebWritable(writable) {
            const success = pipeToStreamWritableWeb(escapeResult, writable);
            assertUsage(success, '`pageContext.httpResponse.pipeToWebWritable` is not available: make sure your `render()` hook provides a Web Stream Pipe, see https://vite-plugin-ssr.com/html-streaming');
        },
        bodyPipeToNodeWritable(writable) {
            const success = pipeToStreamWritableNode(escapeResult, writable);
            assertUsage(success, '`pageContext.httpResponse.pipeToNodeWritable` is not available: make sure your `render()` hook provides a Node.js Stream Pipe, see https://vite-plugin-ssr.com/html-streaming');
        }
    };
}
async function prerenderPage(pageContext) {
    ...
    assertUsage(renderHookResult.escapeResult !== null, "Pre-rendering requires your `render()` hook to provide HTML. Open a GitHub issue if that's a problem for you.");
...
async function executeAddPageContextHook(pageContext) {
    const onBeforeRender = pageContext._pageServerFile?.fileExports.onBeforeRender ||
...
async function executeRenderHook(pageContext) {
    ...
    if (!isObject(result) || isEscapeInject(result)) {
        ...
        assertUsage(result === null || isEscapeInject(result), [
            errPrefix,
        ...
        if ('documentHtml' in result) {
            ...
            assertUsage(documentHtml === undefined || documentHtml === null || isEscapeInject(documentHtml), [errPrefix, 'returned `{ documentHtml }`, but `documentHtml` should be', errSuffix].join(' '));
        }
    }
