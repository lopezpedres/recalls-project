...
function assertBaseUrl(baseUrl, userErrorMessagePrefix) {
    ...
    assertUsage(baseUrl.startsWith('/') || baseUrl.startsWith('http') || baseUrl.startsWith('./'), userErrorMessagePrefix + 'Wrong `base` value `' + baseUrl + '`; `base` should start with `/`, `./`, or `http`.');
    assertUsage(!baseUrl.startsWith('./'), 'Relative Base URLs are not supported yet (`baseUrl` that starts with `./`). Open a new GitHub ticket so we can discuss adding support for your use case.');
}
